...
ScreenDriverSSD1306::ScreenDriverSSD1306(gpio_num_t pinSCL, gpio_num_t pinSDA, gpio_num_t pinEnable) :
        ScreenDriver(128, 64),
        m_pinSCL(pinSCL), m_pinSDA(pinSDA), m_pinEnable(pinEnable)
{
}
...
//    config.master.clk_speed = 1025 * 1024; // 10000;
    config.master.clk_speed = 900 * 1024; // 10000;
...
ScreenDriverSSD1306::handleReset() {
    ...
    // If there is an enable bin, take it low and the bring it back high
    if (m_pinEnable != GPIO_NUM_NC) {
        gpio_set_direction(m_pinEnable, GPIO_MODE_OUTPUT);
        gpio_set_level(m_pinEnable, 0);

        // 50ms seems like a good time???
        vTaskDelay(50 / portTICK_PERIOD_MS);

        gpio_set_level(m_pinEnable, 1);
        ESP_LOGD(TAG, "screen enable pin %d bounced for 50ms-ish", m_pinEnable);
    } else {
        ESP_LOGD(TAG, "No screen enable pin to set");
    }
